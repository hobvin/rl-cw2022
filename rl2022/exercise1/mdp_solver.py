...
class MDPSolver(ABC):
    ...
    **DO NOT CHANGE THIS CLASS**
    ...
    def __init__(self, mdp: MDP, gamma: float):
        ...
        self.MAXITERATION: int = int(1e+10)
        ...
        for i in range(self.MAXITERATION):
            ...
            for state in range(self.state_dim):
                ...
                for a in range(self.action_dim):
                    sum_v=0
                    for s_ in range(self.state_dim):
                        p = self.mdp.P[state,a,s_]
                        r = self.mdp.R[state,a,s_]
                        sum_v += p*(r+self.gamma*V[s_])
                    new_v.append(sum_v)
                V[state]=max(new_v)
                delta_new=abs(v-V[state])
                delta = max(delta,delta_new)
            if delta < self.theta:
                break
        return V 
    ...
    **YOU NEED TO IMPLEMENT FUNCTIONS IN THIS CLASS**
    ...
    def _policy_eval(self, policy: np.ndarray) -> np.ndarray:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        """
        V = np.zeros(self.state_dim)
        #V = np.random.random(self.state_dim)
        ### PUT YOUR CODE HERE ###
        while True:
            delta=0
            for state in range(self.state_dim):
    ...
    def _policy_improvement(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        **YOU MUST IMPLEMENT THIS FUNCTION FOR Q1**
        ...
        V = np.zeros([self.state_dim])
        #V = np.random.random(self.state_dim)
        #policy = np.random.random([self.state_dim,self.action_dim])
        ### PUT YOUR CODE HERE ###
        ...
        while True:
            ...
            for state in range(self.state_dim):
                v=V[state]
                new_v=[]
                old_action=list(policy[state,:]).index(1.0)
                for a in range(self.action_dim):
                    sum_v=0
                    for s_ in range(self.state_dim):
                        p=self.mdp.P[state,a,s_]
                        r=self.mdp.R[state,a,s_]
                        sum_v+=p*(r+self.gamma*V[state])
                    new_v.append(sum_v)
                for j in range(len(new_v)):
                    if new_v[j]==max(new_v):
                        policy[state,:]=0
